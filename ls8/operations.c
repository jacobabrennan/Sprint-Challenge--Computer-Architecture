...
{
    (void)(operand_2);
    cpu->registers[REGISTER_INTERRUPT_STATUS] |= cpu->registers[operand_1];
}
...
{
    (void)(operand_2);
    if(cpu->FL & (CMP_FLAG_LESS|CMP_FLAG_EQUAL))
    {
        cpu->PC = cpu->registers[operand_1];
    }
    else
...
{
    (void)(operand_2);
    if(cpu->FL & (CMP_FLAG_GREATER|CMP_FLAG_EQUAL))
    {
        cpu->PC = cpu->registers[operand_1];
    }
    else
...
{
    (void)(operand_2);
    cpu->registers[operand_1] = ~cpu->registers[operand_1];
}
